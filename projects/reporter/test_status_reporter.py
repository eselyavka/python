...
from projects.reporter import reporter
from projects.reporter.status_reporter.rate import SrvRate
from projects.reporter.status_reporter.report import Report
...
class TestReport(unittest.TestCase):
    def setUp(self):
        self.report = Report()
        self.report.get_raw_storage().clear()
    ...
    def test_report_store(self):
        ...
        self.assertListEqual(list(report_store[apps[0]].keys()), [versions[0]])
        self.assertListEqual(list(report_store[apps[2]].keys()), [versions[0]])
        self.assertListEqual(list(report_store[apps[1]].keys()), versions)
...
class TestStatusReport(unittest.TestCase):
    def setUp(self):
        ...
        reporter.REPORT.get_raw_storage().clear()
        reporter.NEED_ATTENTION_SRV.clear()
    ...
    def test_request_sever_should_return_false_on_requests_exception(self):
        with mock.patch('projects.reporter.reporter.requests.request') as mock_requests:
            mock_requests.side_effect = reporter.requests.exceptions.HTTPError('unittest')
    ...
    def test_requester_should_populate_need_attention_set_when_error_in_request_server(self):
        apps = ['app1', 'app2']
        with mock.patch('projects.reporter.reporter.request_server') as mock_request_server:
    ...
    def test_requester_should_populate_need_attention_set_when_error_in_process_request(self):
        ...
        with mock.patch('projects.reporter.reporter.request_server') as mock_request_server:
            ...
            with mock.patch('projects.reporter.reporter.process_request') as mock_process_request:
