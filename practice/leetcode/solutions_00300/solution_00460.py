...
class TestLFUCache(unittest.TestCase):
    ...
    def test_lfu_1(self):
        ...
        """
        for x in range(1, 4):
            self.lfu_cache.put(f'val{x}', 10 * x)
        for _ in range(10):
            self.lfu_cache.get('val1')
        for _ in range(20):
            self.lfu_cache.get('val2')
        self.lfu_cache.put('val4', 100)
        self.assertEqual(self.lfu_cache.get('val3'), -1)
        lfu_keys = sorted(self.lfu_cache.cache.keys())
        lfu_hash_table_weights = sorted(self.lfu_cache.count.values())
    ...
    def test_lfu_2(self):
        ...
        for _ in range(10):
            self.lfu_cache.get('val2')
        for _ in range(10):
            self.lfu_cache.get('val3')
        self.lfu_cache.put('val4', 100)
        lfu_hash_table_weights = sorted(self.lfu_cache.count.values())
        self.assertTrue('val2' in self.lfu_cache.cache or
                        'val3' in self.lfu_cache.cache)
        self.assertTrue(all(x in self.lfu_cache.cache for x in ['val1', 'val4']))
        self.assertEqual(lfu_hash_table_weights, [1, 11, 21])
