...
class Solution(object):
    ...
    @staticmethod
    def _traverse(node, val_by_levels, level=1):
        if node:
            if level in val_by_levels:
                val_by_levels[level].append(node.val)
            else:
                val_by_levels[level] = [node.val]
            Solution._traverse(node.left, val_by_levels, level=level + 1)
            Solution._traverse(node.right, val_by_levels, level=level + 1)

    def averageOfLevels(self, node):
        """
        :type node: TreeNode
        :rtype: List[float]
        """
        val_by_levels = {}
        Solution._traverse(node, val_by_levels)
        res = []
