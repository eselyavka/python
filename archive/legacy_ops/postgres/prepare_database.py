...
"""Prepare a restored PostgreSQL data directory from a base backup."""

# pylint: disable=bad-indentation,too-many-branches,too-many-statements,too-many-nested-blocks
...
USAGE = (
    '%s -a <basebackup tar archive> -d <new PGDATA directory> '
    '[-u <postgres user, default postgres> -g <postgres group, default postgres>, '
    '-c <path to postgres config, default in new PGDATA dir> '
    '-p <port for new pg_cluster, default 5433>]'
)
    ...
    with open(config, 'r+') as fconf:
        ...
        for line in lines:
            if re.search(r'^[#]?port.+', line):
                ...
                replacement = "stats_temp_directory = '/ramdisk/pgstat/" + logFileName + "'"
                fconf.write(re.sub(r'^[#]?stats_temp_directory.+', replacement, line))
            else:
                fconf.write(line)
...
def main(argv):
    ...
    PostgresPort = ''
    try:
      opts, _args = getopt.getopt(argv, "ha:d:u:g:c:p:", ["archive=", "directory=", "user=", "group=", "config=", "port="])
      ...
      if os.path.isabs(BaseBackupArch) and os.path.isabs(Directory):
        ...
        if PostgresConfig:
          if os.path.isfile(PostgresConfig):
            if PostgresPort:
              process_config(PostgresConfig, os.path.basename(Directory.rstrip('/')), PostgresPort)
            else:
              process_config(PostgresConfig, os.path.basename(Directory.rstrip('/')))
          else:
