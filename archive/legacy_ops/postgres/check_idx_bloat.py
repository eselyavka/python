...
"""Module for archive.legacy_ops.postgres.check_idx_bloat."""
...
# pylint: disable=superfluous-parens,unused-variable,consider-using-f-string
...
def usage():
    ...
    print('\t -p DB password')
    print('\t -o DB port')
    print(f'\t -c Critical size of bloating index,table, default is {warning}')
    print(f'\t -w Warning size of bloating index,table, default is {critical}')
...
def parseArg():
    try:
        opts, _args = getopt.getopt(sys.argv[1:], 'h:H:d:u:p:o:w:c:e:v', ["help", "host=", "dbname=", "user=", "password=", "port=", "warning=", "critical=", "entity=", "verbose"])
...
def parseThreashold(threashold):
    ...
    if len(thresholdList) == 1:
        try:
            ...
            logging.debug('Threashold value: %s', threashold)
