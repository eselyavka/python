...
"""Legacy PostgreSQL base-backup verification script."""

# pylint: disable=global-statement,too-many-branches,too-many-statements,too-many-locals
# pylint: disable=too-many-arguments,redefined-outer-name,invalid-name,broad-exception-caught
# pylint: disable=bad-indentation,trailing-whitespace,global-variable-not-assigned
# pylint: disable=fixme,implicit-str-concat,consider-using-with,raise-missing-from
# pylint: disable=use-list-literal,try-except-raise,consider-using-f-string
import getopt
import logging
import os
import shutil
...
try:
    from html import HTML  # type: ignore[attr-defined]  # pylint: disable=no-name-in-module
except ImportError:  # pragma: no cover
    HTML = None

try:
    from . import database
    from . import prepare_database as pd
except ImportError:  # pragma: no cover
    import database
...
def getAbsPath(pg_version):
    path = [f'/usr/lib/postgresql/{pg_version}/bin/{pgBin}', f'/usr/pgsql-{pg_version}/bin']
    for current_path in path:
        if os.access(current_path, os.X_OK):
...
def generateHTML(unionTablesSet, requestedTablesDict, readTablesDict, diffRequestedTableDict = None, diffReadTableDict = None):
    if HTML is None:
        raise ImportError("The 'html' package providing HTML builder is required")
    h = HTML()
    ...
    if diffRequestedTableDict is not None and diffReadTableDict is None:
        for k in list(diffRequestedTableDict.keys()):
            r = t.tr(bgcolor='red')
            r.td(k.split(":")[0])
            r.td(k.split(":")[1])
        ...
        for k in list(diffReadTableDict.keys()):
            r = t.tr(bgcolor='red')
...
def compareStatistics(statistics, fileName, port):
    ...
    try:
        ...
        try:
            ...
            raise IOError(f"Can't open file {fileName}_{port}")

    for d in list(statistics.keys()):
    ...
    for s in requestedStatList:
        requestedTablesDict[s.split(":")[0] + ":" + s.split(":")[1]] = int(s.split(":")[2].replace("\n",""))

    for s in readStatList:
        readTablesDict[s.split(":")[0] + ":" + s.split(":")[1]] = int(s.split(":")[2].replace("\n",""))
...
def stopPostgres():
    ...
    if pgExecutable and os.path.exists(pgExecutable):
       try:
           subprocess.check_call([pgExecutable, 'stop', '-D', Directory, '-m', 'fast'], stdout=null_file, stderr=subprocess.STDOUT)
...
def startPostgres():
    ...
    if pgExecutable and os.path.exists(pgExecutable):
       try:
           subprocess.check_call([pgExecutable, 'start', '-D', Directory], stdout=null_file, stderr=subprocess.STDOUT)
...
def mkDir(path):
    try:
        checkDir(path)
    except DirectoryPathIsNotAbs as e:
...
def chmodDir(path, perm = 0o700):
    try:
        checkDir(path)
        os.chmod(path, perm)
    except DirectoryPathIsNotAbs as e:
        raise DirectoryPathIsNotAbs(e)
    except DirectoryNotExists as e:
...
def main(argv):
    ...
    try:
        opts, _args = getopt.getopt(argv,"h:a:d:u:w:p:v",["help=","archive=","directory=","user","password","port=","version="])
