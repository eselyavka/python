...
"""Module for archive.legacy_ops.integration.log4j_parser."""
...
def check_file(LogFile):
    return os.path.isfile(LogFile) and os.path.isabs(LogFile)
...
def make_report(Report, Time, Recipients, NoTime):
    Msg=''
    for StrNum, Data in list(Report.items()):
...
def main(argv):
    ...
    NoTime = False
    try:
        opts, _args = getopt.getopt(argv,"hf:l:t:n",["file=", "level=", "to=", "notime"])
