...
def data_packer(is_bin, rnd):
    ...
    if is_bin:
        values.insert(0, struct.calcsize(f'<B I I H B H {values[5]}s I Q Q'))
        s_fmt = struct.Struct(f"<H B I I H B H {values[6]}s I Q Q")
...
def data_writer(data_fn, is_bin=False, rnd=False):
    """
        ...
        Returns:
            None
    """
    file_mode = 'wb' if is_bin else 'w'
    with open(data_fn, file_mode) as fh:
        for _ in range(MEGABYTES // BATCH_SIZE_MEGABYTES):
            data = [data_packer(is_bin, rnd) for _ in range((BATCH_SIZE_MEGABYTES << 20) // 39)]
            fh.writelines(data)
            del data
