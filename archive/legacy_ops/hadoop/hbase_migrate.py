...
"""Module for archive.legacy_ops.hadoop.hbase_migrate."""

# pylint: disable=bad-indentation,wrong-import-position,wrong-import-order,wildcard-import
# pylint: disable=deprecated-module,bare-except,consider-using-f-string,logging-not-lazy
# pylint: disable=expression-not-assigned,unidiomatic-typecheck,too-many-branches
...
# config
source_hbase_server = 'src.example.com'
source_thrift_port = 9090
source_hftp_server = 'src-hftp.example.com'
source_hftp_port = 50070
source_hadoop_bin = '/usr/bin/hadoop'
source_hbase_jar = '/usr/lib/hbase/hbase.jar'
source_export_path = '/export' # path in hdfs
source_ssh_port = 22
source_ssh_user = 'hdfs'
dest_hbase_server = 'dst.example.com'
dest_thrift_port = 9090
dest_hadoop_bin = '/usr/bin/hadoop'
dest_hbase_jar = '/usr/lib/hbase/hbase.jar'
dest_import_path = '/import' # path in hdfs
no_of_versions = 2147483647
...
def copy_table(table_name, begin_timestamp="", end_timestamp=""):
    ...
    try:
        ...
        distcp_command = 'su - hdfs -c \'%s distcp -overwrite -p "hftp://%s:%s%s/%s" "hdfs://%s/%s"\'' % (
    ...
    try:
        ...
        column_families = []
        for name,cf in list(desc.items()):
