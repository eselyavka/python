...
"""Module for technical_assessments.geotab."""
...
def risk_score(event_list, rules=None, lo=0, hi=100):
    rules = rules or [SpeedingRule(), BreakRule(), IdleRule()]
    score = sum(sum(rule.score(event) for event in event_list) for rule in rules)
...
def idle_seconds_by_vehicle(event_list):
    """
    ...
    last = event_list[-1]["ts"]
    if acc:
        for vehicle_id, ts in acc.items():
            duration = last - ts
            ans[vehicle_id] += duration
...
if __name__ == "__main__":
    ...
    print(idle_seconds_by_vehicle(idle_events))
